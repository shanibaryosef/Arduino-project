...
class ArduinoCommunication:
    ...
    def __init__(self, port='COM3', baudrate=115200, data_filename='sensor_data.txt'):

        self.serial = serial.Serial(port, baudrate)

        time.sleep(3) # Added a sleep because of the port taking time to respond....
...
class SensorDataPlot:
    ...
    def plot(self):
        ...
        self.ax.set_ylim(0, 1700)

        self.ax.set_title('Distance Sensor Data')

        self.ax.set_xlabel('Measurements')

        self.ax.set_ylabel('Distance Value')

        self.ax.legend(loc='upper right')

        self.ax.grid()  # Add grid to the plot

        ani = animation.FuncAnimation(self.fig, self.update_plot, blit=False, interval=100, repeat=True) # set blit to false so the graph will show.
...
def main():
    ...
    with ArduinoCommunication() as ser:
        ...
        time.sleep(10) # wait 10 seconds for the data to load from the arduino.
